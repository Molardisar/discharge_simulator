...
恒功率放电模拟器是一个基于实验数据的电芯放电仿真工具，使用二维插值算法预测电芯在任意恒功率条件下的电压、电流、温度和 SoC 变化。

### 1.1 核心功能

- **数据导入**：读取多倍率恒流放电 Excel 数据，**智能检测表格结构**（自动识别电流标签行、列名行、数据起始行）
- **二维插值**：构建 V = f(容量，电流) 和 T = f(容量，电流) 插值函数
- **时间步进模拟**：离散化恒功率放电过程
- **温度演化**：**温升速率积分** `T(t+dt) = T(t) + dT/dt × dt`，支持段间连续
- **多段工况**：**链式模拟**，上一段结束 SoC/温度 → 下一段初始状态
- **可视化**：实时绘制 4 子图放电曲线（电压、电流、温度、SoC）
- **结果导出**：CSV 数据 + PNG 图表
...
└─────────────────────────┘
    │
    ▼
(voltage_func, temp_func, current_min, current_max)
    │
...
```
段 1: start_soc=0.95, start_temp=25℃
    │
    ├─ simulate_constant_power(...)
    │   └─ 结果：end_soc=0.70, end_temp=35℃, time=0→1800s
    │
    ▼
段 2: start_soc=0.70, start_temp=35℃ ← 上一段结束状态
    │
    ├─ simulate_constant_power(..., start_temperature=35℃)
    │   └─ 结果：end_soc=0.52, end_temp=39℃, time=1800→1980s
    │
    ▼
段 3: start_soc=0.52, start_temp=39℃
    │
    └─ ...

时间轴：t_offset 累加器
- 段 1: time = 0 → 1800s
- 段 2: time = (0 → 180s) + 1800 = 1800 → 1980s
- 段 3: time = (0 → 30s) + 1980 = 1980 → 2010s
```

### 2.4.1 温度连续性

**问题**：实验数据 T=f(Q,I) 是从 25℃开始测的绝对温度，段间直接插值会导致跳变。

**解决**：温升速率积分
```python
# 从实验数据推导温升速率
dT_dt = (T(Q+dQ, I) - T(Q, I)) / dt
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0.4 | 2026-04-17 | 温度模拟 + 动态电流检测 + 移除硬编码 |
| v1.0.3 | 2026-04-17 | 温度曲线模拟 |
| v1.0.2 | 2026-04-15 | 容量横坐标图表 |
| v1.0.1 | 2026-04-15 | 图表标签英文化 |
